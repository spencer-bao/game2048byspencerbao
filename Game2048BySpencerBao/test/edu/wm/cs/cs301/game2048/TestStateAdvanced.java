...
class TestStateAdvanced {
	...
	final void testSetEmptyBoardAndEquals() {
		...
		State sut1 = new State();
		sut1.setEmptyBoard();
		State sut2 = new State();
		sut2.setEmptyBoard();
	 ...
	 */
	private int findPositionAndFlip(State sut) {
	 ...
	 */
	private State createFullBoard(int value) {
		State sut = new State();
		sut.setEmptyBoard();
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++) {
	...
	private void setBoard(State sut, int[] input) {
	...
	private void getBoard(State sut, int[] output) {
